...
[Source Documentation](https://solana-labs.github.io/solana-web3.js/classes/BpfLoader.html)

The BpfLoader is a native program for deploying, executing, and upgrading programs on chain.
...
let program = web3.Keypair.generate();
let payerAccount = web3.Keypair.generate();
let programData = fs.readFile(/* .so compiled rust program */);

let {feeCalculator} = await connection.getRecentBlockhash();
let fees = feeCalculator.lamportsPerSignature * web3.BpfLoader.getMinNumSignatures(programData.length);
let payerBalance = await connection.getMinimumBalanceForRentExemption(
    0,
);
let executableBalance =
...
let programAirdropSignature = await connection.requestAirdrop(
    payerAccount.publicKey,
    executableBalance - 1,
);

await connection.confirmTransaction(programAirdropSignature);
...
Connection is used in order to interact with the [Solana JSON RPC](https://docs.solana.com/developing/clients/jsonrpc-api). You can use Connection in order to confirm transactions, get account info, and more.
...
The Enum class is used to represent a Rust compatible Enum in javascript. The enum will just be a string representation if logged, but can be properly encoded/decoded when used in conjunction with [Struct](javascript-api.md#Struct).
...
[Source Documentation](https://solana-labs.github.io/solana-web3.js/classes/Loader.html)

Loader is used in [BpfLoader](javascript-api.md#BpfLoader) to load rust compiled programs on chain.

Please refer to [BpfLoader](javascript-api.md#Loader) for usage.
...
Message is used as another way to construct transactions. You can construct a message using the accounts, header, instructions, and recentBlockhash that are a part of a transaction.
...
(async () => {
  ...
  const base58publicKey = new web3.PublicKey('5xot9PVkphiX2adznghwrAuxGs2zeWisNSxMW6hU6Hkj');
  ...
  const programAddressFromKey = await web3.PublicKey.createProgramAddress([highEntropyBuffer.slice(0, 31)], base58publicKey);
  ...
  const validProgramAddress = await web3.PublicKey.findProgramAddress([Buffer.from('', 'utf8')], programAddressFromKey);
...
const plaintextHash = Buffer.from(keccak_256.update(plaintext).digest());
const {signature, recid: recoveryId} = secp256k1.ecdsaSign(
    plaintextHash, 
    secp256k1PrivateKey
);
...
[SourceDocumentation](https://solana-labs.github.io/solana-web3.js/classes/StakeInstruction.html)

StakeInstruction class is used to decode and read more instructions from transactions calling the [StakeProgram](javascript-api.md#StakeProgram).

See [StakeProgram](javascript-api.md#StakeProgram) for more information and usage.

### StakeProgram

[SourceDocumentation](https://solana-labs.github.io/solana-web3.js/classes/StakeProgram.html)

The StakeProgram facilitates staking SOL and delegating them to any validators on the network. You can use StakeProgram to create a stake account, stake some SOL, authorize accounts for withdrawal of your stake, deactive your stake, and withdraw your funds.
...
[SourceDocumentation](https://solana-labs.github.io/solana-web3.js/classes/SystemInstruction.html)

SystemInstruction class is used to decode and read more instructions from transactions calling the [SystemInstruction](javascript-api.md#SystemInstruction).

See [SystemInstruction](javascript-api.md#SystemInstruction) for more information and usage.

### SystemProgram

[SourceDocumentation](https://solana-labs.github.io/solana-web3.js/classes/SystemProgram.html)

The SystemProgram grants the ability to create accounts, allocate account data, assign account to programs, work with nonce accounts, and transfer lamports.
...
[SourceDocumentation](https://solana-labs.github.io/solana-web3.js/classes/TransactionInstruction.html)

The TransactionInstruction object is used to create instructions to be used in transactions. They consist of keys, data, and a programId.

See [Transaction](javascript-api.md#Transaction) for more use.

### ValidatorInfo

[SourceDocumentation](https://solana-labs.github.io/solana-web3.js/classes/ValidatorInfo.html)

Validator Info is an object that makes it easy to decode the program account data that sets the validators currently on the network. You can get the validators account data from the native Config program and decode to see more information about the validators.
...
// You can get current validators from the configProgram
let configAccounts = await connection.getProgramAccounts(new web3.PublicKey('Config1111111111111111111111111111111111111'));

let infos = configAccounts.map((configAccount) =>
web3.ValidatorInfo.fromConfigData(configAccount.account.data))

console.log(infos[0])

ValidatorInfo {
  key: PublicKey {
    _bn: <BN: fd084b7c56c430606ffd948133bd98af1b126e077200ca5440114d2b2b65894e>
  },
  info: {
    details: 'Aquanow is operating validator nodes in support of emerging decentralized finance projects.',
    keybaseUsername: 'aquavalidate',
    name: 'Aquanode',
    website: 'https://aquanow.io'
  }
}
```
